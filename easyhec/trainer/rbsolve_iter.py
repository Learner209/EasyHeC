...
class RBSolverIterTrainer(BaseTrainer):
    ...
    def fit(self, **kwargs):
        ...
        # cfg.model.rbsolver_iter.start_qpos = qposes[0,:7]
        for ind, explore_it in enumerate(sampled_inds):
            ...
            self.explore_next_state(explore_it, explore_it, qposes[explore_it])
            self.capture_data(
                index=ind,
                fake_img_path=batch_imgs_paths[explore_it],
                fake_camera_intrinsics=camera_intrinsics[explore_it],
                fake_camera_extrinsics=gt_local_to_world_matrices[explore_it],
                local_save_path=local_save_path,
                fake_gt_mask=gt_mask,
            )
            self.rebuild()
    ...
    def capture_data(
        ...
        if fake_camera_extrinsics is None:
            fake_camera_extrinsics = np.eye(4)

        np.savetxt(osp.join(outdir, "Tc_c2b.txt"), fake_camera_extrinsics)  # fake ground-truth Tc_c2b
        ...
        # cv2.imshow("fake_gt_mask", fake_gt_mask)
        # cv2.waitKey(10)
        # cv2.destroyAllWindows()

        mask_outpath = osp.join(outdir, "mask", osp.basename(image_path))
        os.makedirs(osp.dirname(mask_outpath), exist_ok=True)
        gt_mask_outpath = osp.join(outdir, "gt_mask", osp.basename(image_path))
        os.makedirs(osp.dirname(gt_mask_outpath), exist_ok=True)
        imageio.imsave(mask_outpath, pred_binary_mask)
        imageio.imsave(gt_mask_outpath, fake_gt_mask)
